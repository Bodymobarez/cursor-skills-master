...
export async function acquirePage(url, options = {}) {
  ...
  try {
    ...
    // In interactive mode, default to headful unless caller overrides explicitly.
    if (interactive && launchOptions.headless === undefined) {
      // no-op here; openBrowser already received launchOptions
    }
    ...
    // Many sites render core content around H1.
    ...
    if (debug) {
      ...
      console.error(`[puppeteer.acquirePage] ${url} failed: ${msg}`);
    }
    return null;
  } finally {
    await browser.close();
